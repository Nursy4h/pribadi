...
def log(value, status='INFO Local Host', color='[R2]'):
...
class inject(object):
    ...
    def start(self):
        try:
            ...
            frontend_domains = open(real_path('/bug.txt')).readlines()
            frontend_domains = filter_array(frontend_domains)
            if len(frontend_domains) == 0:
                self.log('Frontend Domains not found. Please check bug.txt', color='[Y1]')
                return
...
class domain_fronting(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.log('HTTP/1.0 200 Established '.format(self.proxy_host, self.proxy_port))
            ...
            self.log('HTTP/1.1 200 OK ''[R2]'.format(self.proxy_host, self.proxy_port), color='[R1]')
        except OSError:
